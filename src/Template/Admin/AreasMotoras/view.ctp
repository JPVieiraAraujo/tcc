...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Edit Areas Motora'), ['action' => 'edit', $areasMotora->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Areas Motora'), ['action' => 'delete', $areasMotora->id], ['confirm' => __('Are you sure you want to delete # {0}?', $areasMotora->id)]) ?> </li>
...
<div class="areasMotoras view large-9 medium-8 columns content">
    <h3><?= h($areasMotora->id) ?></h3>
    ...
    <div class="row">
        <h4><?= __('Texto') ?></h4>
        <?= $this->Text->autoParagraph(h($areasMotora->texto)); ?>
    </div>
    <div class="related">
        <h4><?= __('Related Avals') ?></h4>
        <?php if (!empty($areasMotora->avals)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($areasMotora->avals as $avals): ?>
            <tr>
                <td><?= h($avals->id) ?></td>
